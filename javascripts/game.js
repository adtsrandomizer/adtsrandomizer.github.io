...
function setRandomizerMode(mode) {
    switch(mode) {
        case 1:
            document.getElementById("randmode").innerHTML = "Price Randomizer Mode: 7/8 to 9/8 of Vanilla position price"
            player.randomizerMode = 1;
            break;
        case 2:
            document.getElementById("randmode").innerHTML = "Price Randomizer Mode: 3/4 to 5/4 of Vanilla position price"
            player.randomizerMode = 2;
            break;
        case 3:
            document.getElementById("randmode").innerHTML = "Price Randomizer Mode: 1/2 to 3/2 of Vanilla position price"
            player.randomizerMode = 3;
            break;
        case 4:
            document.getElementById("randmode").innerHTML = "Price Randomizer Mode: 1 TT to x2 of Vanilla position price"
            player.randomizerMode = 4;
            break;
        case 5:
            document.getElementById("randmode").innerHTML = "Price Randomizer Mode: 1 TT to Vanilla position price"
            player.randomizerMode = 5;
            break;
        case 6:
            document.getElementById("randmode").innerHTML = "GOOD LUCK Randomizer: Vanilla price of time study (NOT POSITION)"
            player.randomizerMode = 6;
            break;
    }
}
...
document.getElementById("randmode").onclick = function () {
    if(player.randomizerMode != 6) setRandomizerMode(player.randomizerMode + 1);
    else setRandomizerMode(1);
}
...
function getGalaxyCostScalingStart() {
    ...
    if (player.effectiveStudies.includes(223)) n += 7
    if (player.effectiveStudies.includes(224)) n += Math.floor(player.resets/2000)
    return n
}

function getGalaxyRequirement() {
    let amount = 80 + ((player.galaxies) * 60);
    if (player.effectiveStudies.includes(42)) amount = 80 + ((player.galaxies) * 52)
...
function updateDimensions() {
    if (document.getElementById("antimatterdimensions").style.display == "block" && document.getElementById("dimensions").style.display == "block") {
        ...
        if (player.effectiveStudies.includes(225)) extraGals += Math.floor(player.replicanti.amount.e / 1000)
        if (player.effectiveStudies.includes(226)) extraGals += Math.floor(player.replicanti.gal / 15)
        var galString = ""
    ...
    if (document.getElementById("infinity").style.display == "block") {
        if (document.getElementById("preinf").style.display == "block") {
            ...
            if (player.effectiveStudies.includes(31)) document.getElementById("postinfi12").innerHTML = "Power up all dimensions based on amount infinitied <br>Currently: "+shortenMoney(Math.pow((Math.log10(getInfinitied()+1)*10).toFixed(2), 4))+"x<br>Cost: "+shortenCosts(1e5)+" IP"
...
function updateInfCosts() {
    ...
    if (document.getElementById("replicantis").style.display == "block" && document.getElementById("infinity").style.display == "block") {
        ...
        let replGalOver = 0
        if (player.effectiveStudies.includes(131)) replGalOver += Math.floor(player.replicanti.gal / 2)
        if (player.effectiveStudies.includes(233)) {
        ...
        document.getElementById("replicantiinterval").className = (player.infinityPoints.gte(player.replicanti.intervalCost) && ((player.replicanti.interval !== 50) || player.effectiveStudies.includes(22)) && (player.replicanti.interval !== 1)) ? "storebtn" : "unavailablebtn"
        ...
        document.getElementById("replicantireset").className = ((player.replicanti.galaxies < player.replicanti.gal && player.replicanti.amount.gte(Number.MAX_VALUE)) || (player.replicanti.galaxies < Math.floor(player.replicanti.gal * 1.5) && player.replicanti.amount.gte(Number.MAX_VALUE) && player.effectiveStudies.includes(131))) ? "storebtn" : "unavailablebtn"
    ...
    if (document.getElementById("timestudies").style.display == "block" && document.getElementById("eternitystore").style.display == "block") {
        ...
        document.getElementById("201desc").innerHTML = (player.dilation.upgrades.includes(8)) ? "<b>Dil8 has been bought, making this TS useless.</b>" : ""
        ...
        document.getElementById("ec11unl").innerHTML = "Eternity Challenge 11<span>Requirement: Use only the Green path<span>Cost: 1 Time Theorem"
        document.getElementById("ec12unl").innerHTML = "Eternity Challenge 12<span>Requirement: Use only the Purple path<span>Cost: 1 Time Theorem"
...
function upgradeReplicantiInterval() {
    if (player.infinityPoints.gte(player.replicanti.intervalCost) && (player.replicanti.interval > 50 || player.effectiveStudies.includes(22)) && player.replicanti.interval !== 1 && player.eterc8repl !== 0) {
        ...
        if (!player.effectiveStudies.includes(22) && player.replicanti.interval < 50) player.replicanti.interval = 50
        if (player.effectiveStudies.includes(22) && player.replicanti.interval < 1) player.replicanti.interval = 1
...
function upgradeReplicantiGalaxy() {
    ...
    if (player.effectiveStudies.includes(233)) cost = cost.dividedBy(player.replicanti.amount.pow(0.3))
...
function replicantiGalaxy() {
    if (player.replicanti.amount.gte(Number.MAX_VALUE) && (!player.effectiveStudies.includes(131) ? player.replicanti.galaxies < player.replicanti.gal : player.replicanti.galaxies < Math.floor(player.replicanti.gal * 1.5))) {
...
function replicantiGalaxyAutoToggle() {
    if (player.replicanti.galaxybuyer) {
        ...
        if (player.effectiveStudies.includes(131)) document.getElementById("replicantiresettoggle").textContent = "Auto galaxy OFF (disabled)"
        else document.getElementById("replicantiresettoggle").textContent = "Auto galaxy OFF"
    } else {
        player.replicanti.galaxybuyer = true
        if (player.effectiveStudies.includes(131)) document.getElementById("replicantiresettoggle").textContent = "Auto galaxy ON (disabled)"
...
document.getElementById("two_eternity_save").onclick = function () {
    if(confirm("Pressing OK will reset everything and load a template save with 2 Eternities. Your current save file will be overwritten. Continue?")) {
        importsave("");
    }
}
...
function gainedInfinityPoints() {
    let div = 308;
    if (player.effectiveStudies.includes(111)) div = 285;
    else if (player.achievements.includes("r103")) div = 307.8;

    var ret = Decimal.pow(10, player.money.e/div -0.75).times(player.infMult)
    if (player.effectiveStudies.includes(41)) ret = ret.times(Decimal.pow(1.2, player.galaxies + player.replicanti.galaxies))
    if (player.effectiveStudies.includes(51)) ret = ret.times(1e15)
    if (player.effectiveStudies.includes(141)) ret = ret.times(new Decimal(1e45).dividedBy(Decimal.pow(15, Math.log(player.thisInfinityTime+1)*Math.pow(player.thisInfinityTime+1, 0.125))).max(1))
    if (player.effectiveStudies.includes(142)) ret = ret.times(1e25)
    if (player.effectiveStudies.includes(143)) ret = ret.times(Decimal.pow(15, Math.log(player.thisInfinityTime+1)*Math.pow(player.thisInfinityTime+1, 0.125)))
...
function gainedEternityPoints() {
    ...
    if (player.effectiveStudies.includes(61)) ret = ret.times(10)
    if (player.effectiveStudies.includes(121)) ret = ret.times(((253 - averageEp.dividedBy(player.epmult).dividedBy(10).min(248).max(3))/5)) //x300 if tryhard, ~x60 if not
    else if (player.effectiveStudies.includes(122)) ret = ret.times(35)
    else if (player.effectiveStudies.includes(123)) ret = ret.times(Math.sqrt(1.39*player.thisEternity/10))
...
function calcSacrificeBoost() {
    ...
    if (player.challenges.includes("postc2")) {
        if (player.effectiveStudies.includes(228)) return player.firstAmount.dividedBy(player.sacrificed.max(1)).pow(0.013).max(1)
...
function calcTotalSacrificeBoost() {
    ...
    if (player.challenges.includes("postc2")) {
        if (player.effectiveStudies.includes(228)) return player.sacrificed.pow(0.013).max(1)
...
document.getElementById("bigcrunch").onclick = function () {
    ...
    if ((player.money.gte(Number.MAX_VALUE) && !player.currentChallenge.includes("post")) || (player.currentChallenge !== "" && player.money.gte(player.challengeTarget))) {
        ...
        if (!player.break || player.currentChallenge != "") {
            ...
            if (player.effectiveStudies.includes(51)) add = add.times(1e15)
        ...
        if (player.effectiveStudies.includes(32)) infGain *= Math.max(player.resets,1);
        ...
        player.replicanti.galaxies = (player.effectiveStudies.includes(33)) ? Math.floor(player.replicanti.galaxies/2) :0
        ...
        if (player.eternities >= 60 && player.replicanti.auto[1] && player.currentEternityChall !== "eterc8") {
            while (player.infinityPoints.gte(player.replicanti.intervalCost) && player.currentEternityChall !== "eterc8" && ((player.effectiveStudies.includes(22)) ? player.replicanti.interval > 1 : player.replicanti.interval > 50)) upgradeReplicantiInterval()
        }
...
function eternity(force, auto) {
    if ((player.infinityPoints.gte(Number.MAX_VALUE) && (!player.options.eternityconfirm || auto || confirm("Eternity will reset everything except achievements and challenge records. You will also gain an Eternity point and unlock various upgrades."))) || force === true) {
        ...
        if (player.effectiveStudies.includes(191)) player.infinitiedBank += Math.floor(player.infinitied*0.05)
...
function unlockEChall(idx) {
    if(idx == 11) giveAchievement("If you think it is: it's not.")
    if (player.eternityChallUnlocked == 0) {
...
function gameLoop(diff) {
    ...
    if (player.timeShards.gt(0)) {
        if (player.effectiveStudies.includes(171)) gain = Math.ceil(new Decimal(player.timeShards).dividedBy(player.tickThreshold).log10() / Math.log10(1.25))
        ...
        if (player.effectiveStudies.includes(171)) player.tickThreshold = new Decimal(1).times(1.25).pow(player.totalTickGained)
    ...
    if (player.effectiveStudies.includes(62)) interval = interval/3
    if (player.effectiveStudies.includes(133) || player.replicanti.amount.gt(Number.MAX_VALUE)) interval *= 10
    if (player.effectiveStudies.includes(213)) interval /= 20
    ...
    if (player.replicanti.unl && (diff > 5 || interval < 50 || player.effectiveStudies.includes(192))) {
        var gained = Decimal.pow(Math.E, current +(diff*est/10))
        if (player.effectiveStudies.includes(192)) gained = Decimal.pow(Math.E, current +Math.log((diff*est/10) * (Math.log10(1.2)/308)+1) / (Math.log10(1.2)/308))
        player.replicanti.amount = Decimal.min(Number.MAX_VALUE, gained)
        if (player.effectiveStudies.includes(192)) player.replicanti.amount = gained
        replicantiTicks = 0
    } else {
        if (interval <= replicantiTicks && player.replicanti.unl) {
            if (player.replicanti.amount.lte(100)) {
                ...
                if (Math.round(player.replicanti.chance) !== 1) {
                    ...
                    if (player.effectiveStudies.includes(192)) player.replicanti.amount = temp.times(counter).plus(player.replicanti.amount)
                    counter = 0
                } else {
                    if (player.effectiveStudies.includes(192)) player.replicanti.amount = player.replicanti.amount.times(2)
    ...
    if (player.replicanti.galaxybuyer && player.replicanti.amount.gte(Number.MAX_VALUE) && !player.effectiveStudies.includes(131)) {
        document.getElementById("replicantireset").click()
    }
    if (player.effectiveStudies.includes(22) ? player.replicanti.interval !== 1 : (player.replicanti.interval !== 50)) document.getElementById("replicantiinterval").innerHTML = "Interval: "+(interval).toFixed(3)+"ms<br>-> "+Math.max(interval*0.9).toFixed(3)+" Costs: "+shortenCosts(player.replicanti.intervalCost)+" IP"
    ...
    if (player.effectiveStudies.includes(21)) replmult = replmult.plus(Decimal.pow(player.replicanti.amount, 0.032))
    if (player.effectiveStudies.includes(102))replmult = replmult.times(Decimal.pow(5, player.replicanti.galaxies, 150))
    ...
    document.getElementById("ec10reward").textContent = "Reward: Time dimensions gain a multiplier from infinitied stat, Currently: "+shortenMoney(new Decimal(Math.max(Math.pow(getInfinitied(), 0.9) * ECTimesCompleted("eterc10") * 0.000002+1, 1)).pow((player.effectiveStudies.includes(31)) ? 4 : 1))+"x "
    ...
    // let extraGals = 0
    // if (player.effectiveStudies.includes(225)) extraGals += Math.floor(player.replicanti.amount.e / 2500)
    // if (player.effectiveStudies.includes(226)) extraGals += Math.floor(player.replicanti.gal / 40)
    // if (extraGals !== 0) document.getElementById("replicantireset").innerHTML = "Reset replicanti amount, but get a free galaxy<br>"+player.replicanti.galaxies + "+"+extraGals+ " replicated galaxies created."
    ...
    if (player.effectiveStudies.includes(181)) player.infinityPoints = player.infinityPoints.plus(gainedInfinityPoints().times(diff/1000))
...
setInterval( function() {
    ...
    if (player.effectiveStudies.includes(31)) infinitiedMult = 1 + Math.pow(Math.log10(getInfinitied()+1)*10, 4)
    ...
    if (player.currentEternityChall == "eterc10") {
        ...
        if (player.effectiveStudies.includes(31)) ec10bonus = ec10bonus.pow(4)
    } else {
        ec10bonus = new Decimal(1)
    }
}, 100)
