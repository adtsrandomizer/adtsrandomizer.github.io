function getDimensionBoostPower() {
  ...
  if (player.currentChallenge == "postc7" || player.effectiveStudies.includes(81)) ret = 10

  if (player.achievements.includes("r101")) ret = ret*1.01
  if (player.effectiveStudies.includes(83)) ret = Decimal.pow(1.0004, player.totalTickGained).min(1e30).times(ret);
  if (player.effectiveStudies.includes(231)) ret = Decimal.pow(player.resets, 0.3).times(ret)
...
function getShiftRequirement(bulk) {
  ...
  let mult = 15
  if (player.effectiveStudies.includes(211)) mult -= 5
