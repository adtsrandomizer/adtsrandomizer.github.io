...
if (player.infinitied > 0 && !player.challenges.includes("challenge1")) player.challenges.push("challenge1")
  ...
  setRandomizerMode(player.randomizerMode);
...
if (player.version < 5) {
  ...
  if (player.version < 9.5) {
      player.version = 9.5
      if (player.timestudy.effectiveStudies.includes(191)) player.timestudy.theorem += 100
  }

  if (player.version < 10) {
      player.version = 10
      if (player.timestudy.effectiveStudies.includes(72)) {
          for (i=4; i<8; i++) {
