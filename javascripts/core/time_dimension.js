...
function getTimeDimensionPower(tier) {
  ...
  if (player.effectiveStudies.includes(11) && tier == 1) ret = ret.dividedBy(player.tickspeed.dividedBy(1000).pow(0.005).times(0.95).plus(player.tickspeed.dividedBy(1000).pow(0.0003).times(0.05)).max(Decimal.fromMantissaExponent(1, -2500)))
  ...
  if (player.effectiveStudies.includes(73) && tier == 3) ret = ret.times(calcTotalSacrificeBoost().pow(0.005).min(new Decimal("1e1300")))
  if (player.effectiveStudies.includes(93)) ret = ret.times(Decimal.pow(player.totalTickGained, 0.25).max(1))
  if (player.effectiveStudies.includes(103)) ret = ret.times(Math.max(player.replicanti.galaxies, 1))
  if (player.effectiveStudies.includes(151)) ret = ret.times(1e4)
  if (player.effectiveStudies.includes(221)) ret = ret.times(Decimal.pow(1.0025, player.resets))
  if (player.effectiveStudies.includes(227) && tier == 4) ret = ret.times(Math.max(Math.pow(calcTotalSacrificeBoost().log10(), 10), 1))
  if (hasBuff('3')) ret = ret.times((Decimal.pow(Math.max(player.infinityPower.log2(), 1), 4)).max(1))
  ...
  if (player.replicanti.unl && player.replicanti.amount.gt(1) && player.dilation.upgrades.includes(5)) {
    ...
    if (player.effectiveStudies.includes(102)) replmult = replmult.times(Decimal.pow(5, player.replicanti.galaxies))
